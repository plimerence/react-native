...
/// 下拉刷新的开关
@property (nonatomic, assign) BOOL isPullToRefreshEnd;
@end
...
- (instancetype)initWithEventDispatcher:(RCTEventDispatcher *)eventDispatcher
{
  ...
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    _scrollListeners = [NSHashTable weakObjectsHashTable];
      self.refreshHeader=[[YiRefreshHeader alloc] init];
      self.refreshHeader.scrollView=_scrollView;
      [self.refreshHeader header];
    
      //typeof(self) __weak weakSelf = self;
    
      self.refreshHeader.beginRefreshingBlock=^(){
        [[ZZHScrollViewMes allocWithZone:nil] isRefreshing:@"refreshing"];
        // 后台执行：
//        dispatch_async(dispatch_get_global_queue(0, 0), ^{
//        });
      };
...
- (void)setIsPullToRefreshEnd:(BOOL)isPullToRefreshEnd{
  if (isPullToRefreshEnd) {
    dispatch_async(dispatch_get_main_queue(), ^{
      [self.refreshHeader endRefreshing];});
  }
}
- (void)setActiveRefresh:(BOOL)activeRefresh{
  if (activeRefresh) {
